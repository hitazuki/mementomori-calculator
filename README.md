...
> 包含：单体计算器 · 穿透扫描曲线 · 贯通×防御热力图 · 多方案对比 · 交叉拉表导出 · 圣遗物（神装）模拟评分计算器
...
│   │   └── mysterium_data.json    # 圣遗物藏品数据（用于秘仪计算）
│   │
│   ├── engine/
│   │   ├── damageCalc.js          # 【核心逻辑】真实双路贯通减伤公式计算
│   │   └── mysteriumCalc.js       # 【核心逻辑】圣遗物组合性价比算法
│   │
│   ├── i18n/                      # Vue-i18n 多语言系统
│   ├── utils/
...
│       └── MysteriumPanelView.vue # 模块六：圣遗物配置评级
```
...
### 🔮 六、圣遗物配置评级 (`MysteriumPanelView.vue`)

针对游戏内的“秘仪”（Mysterium/神装）系统提供的收益评估和配对算法。

**使用方法：**
- **自定权重体系**：左侧提供了高度可定制的评分模板，包括固定值、百分比、成长属性以及各词条的具体权重分数。你可以完全基于你所在的战区环境进行微调。
- **多维度排解算法**：
  - **藏品性价比榜（Collection Rank）**：罗列了每个藏品单体的最高性价比。
  - **独立分配算法（Algo 1）**：把藏品分数通过需求角色的造价比例分配给角色个体。
  - **组件最优化组合（Algo 3）**：寻找需求网络中的强连通组合并评估，指出最值得投资的圣遗物套装及造价瓶颈角色。
