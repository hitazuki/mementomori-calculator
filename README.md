...
> 包含：单体计算器 · 穿透扫描曲线 · 贯通×防御热力图 · 多方案对比 · 交叉拉表导出
...
```
mmt-calculator/
├── index.html                     # 主页面（ECharts CDN + Google Fonts）
├── src/
│   ├── main.js                    # 应用入口，视图路由
...
│   │
│   └── views/                     # 五大功能模块
│       ├── Calculator.js          # 模块一：单体计算器
│       ├── SweepChart.js          # 模块二：穿透扫描曲线
│       ├── HeatmapChart.js        # 模块三：贯通×防御 热力图
│       ├── ComparePanel.js        # 模块四：多方案配装对比
│       └── TableExport.js         # 模块五：交叉拉表 & 导出
```
...
### 🎯 一、单体计算器 (`Calculator.js`)

最基础的计算面板，输入攻击方和防守方的所有详细参数，实时展示拆解过程。

**使用方法：**
- **攻击类型切换**：点击「物理攻击(P.DEF)」或「魔法攻击(M.DEF)」，系统会自动在防守方参数中加载正确的对应的物理/魔法防御定数。
- **攻防双端等级预设**：引入了双端独立等级架构。攻击方等级预设会直接影响 `C_pen` / `C_pmpen` (贯通定数)，防守方等级预设会直接影响 `C_def` / `C_pmdef` (防御定数)，两者互不干涉。
- **结果查看**：右侧「伤害拆解」区展示了由表攻开始，通过「增伤」「防御路」「物魔路」和「爆击」的瀑布式缩减/放大过程。底部「快查表」提供不同目标防御档位在当前穿透下的直观减伤率。

---

### 📈 二、穿透扫描曲线 (`SweepChart.js`)
...
- **设置范围**：设定下限，上限和采样点数。
- **固定参数预设**：在下方设定未扫描参数的定值，支持分别设定攻防独立等级基准。图表将描绘在整个堆叠的过程中，你的**综合穿透率**是如何爬升的。
- 适合用来判断防穿或物魔穿是否有**边际收益递减**甚至溢出的情况。

---

### 🔥 三、贯通 × 防御 热力图 (`HeatmapChart.js`)
...
- **独立常数预设**：侧边栏将 X轴 和 Y轴 的常数基准分离，可独立指定计算 Y轴 贯通收益时的攻击方等级基准，以及计算 X轴 目标防御强度时的防守方等级基准。
- **色阶指示**：深色（深蓝/紫）表示严重刮痧，亮色（金/红）表示伤害通过率高。
- 点击图表右上角的 `⬇ 保存PNG` 可以直接导出高质量配图。

---

### ⚖ 四、多方案配装对比 (`ComparePanel.js`)
...
### 📋 五、交叉拉表与导出 (`TableExport.js`)

为硬核玩家或做攻略的 UP 主准备的数据矩阵生成器。

**使用方法：**
- 设定 X 轴（如：目标防御）和 Y 轴（如：防御贯通）的取值集合（用英文逗号隔开，如 `100万, 500万, 1000万`）。
- **全局基础环境自定义**：侧边栏提供了完全可交互的固定参数输入区，无论是调整攻击力、穿透、破防Debuff，还是分别设置攻防等级，都能作为全局背景参数代入矩阵计算。
- 系统会自动渲染出一张两两对应的矩阵表，格子里的数值可以切换为“综合伤害通过率%”、“最终伤害”或单路“减伤率”。
- 点击 `复制 Markdown` 或 `导出 CSV`，即可一键将矩阵带走到飞书/Notion/Excel 中编辑。
...
没有引用臃肿的 UI 框架（如 Tailwind 或 ElementUI），采用纯原生 CSS (`style.css`) 构建，最大程度保证了定制化、性能和动态模糊（Glassmorphism）效果。
