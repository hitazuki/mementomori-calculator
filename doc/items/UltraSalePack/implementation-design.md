...
**大额累充强制跨日（算法剪枝优化）**：由于单日硬冲超过 12000 的边际性价比显著下降，默认规划目标以每日 12000 付费钻为上界。当当前累计付费钻 `dailyPaidDiamonds >= 12000` 时，算法强制在执行下一批次前跨日重置；如果下一次购买会把当前累充日从 12000 以下直接顶过该上界，也会先在批次边界评估补累充并跨日，再执行该购买。单个购买动作本身也不会合并出超过该日目标的付费钻总量。这属于一种剪枝优化，用于引导规划器自然偏向多日重复凑 12000 并防止状态空间在低性价比分支上无意义展开。
...
2. **跨日补包判定**：当一个批次需要跨日（或同日无后续动作）时，检查当前 `dailyPaidDiamonds` 距离 12000 日目标内下一档 `nextTier` 的差额 `gap`。
3. **轻量参考组合与盈余检查**：
...
实现上会在 `buildPlanningContext` 阶段先按付费钻等次归并常驻钻石组合包，并为 `0..dailyRechargeResetPaid` 的每个当前付费钻数预计算一次补累充参考方案。DP 搜索中 `findPermanentTopUpOption` 只按 `dailyPaidDiamonds` 查表，避免在大量状态上重复分组、排序和贪心补齐。该优化不改变补包启发式结果，只把单次状态判断从按常驻包数与缺口循环降为 O(1) 查表；前端常见的“5900 后补 80×2 / 11800 后补 80×3”只是查表结果之一，不能硬编码为唯一规则。
