...
| --- | --- | --- | --- | --- | --- |
| 魔女的书库大扫除商城 | 完备 | 完备 | 大多完备 | 可计算 | CE 为每 1 个树影书签可兑换的钻石价值；三种封印钥匙限购仍未截图确认 |
| 精品店 | 截图可见 10 个礼盒/礼袋内容完备 | 完备 | 完备 | 可计算 | 货币为免费钻石；CE 为每 1 免费钻石可兑换的道具钻石价值 |

## 待确认

1. 魔女的书库大扫除商城中三种封印钥匙是否视为不限购，还是继续记为 `limitTotal = null`。
2. 后续购买规划功能若需要满购统计，可由 `cost × limitTotal` 与 `reward × limitTotal` 动态计算，不在当前展示需求中直接呈现。
