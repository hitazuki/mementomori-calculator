...
本工具由项目安装页面、公开序列码数据和 Tampermonkey 用户脚本组成。用户脚本只在 MementoMori 官方兑换页运行，支持多账号持久化、按过期时间选择兑换码批次、串行兑换和本地备份。
...
6. 官网接口请求超过 15 秒未响应时按失败处理，不会无限停留在“核对中”。“已经使用”“已兑换完毕”等终态提示会保存记录并继续；其他 HTTP 400、403、500 等错误也只标记当前代码失败，等待 2 秒后继续后续代码。超时、网络错误或玩家信息不一致会跳过当前账号剩余代码，但继续下一个账号。
7. 本机已有“成功”或“官网确认已使用”记录的 `serverId:playerId:CODE` 组合自动跳过，不再向官网重试。
8. 点击“停止本次兑换”后，脚本会在当前请求结束后停止，不保存任务队列。刷新页面或再次点击“开始兑换”会根据当前选择重新计算任务，并自动跳过本机已有“成功”或“已使用”记录的组合。

以下情况会中断流程：

- 未识别为“已使用”等业务终态的 HTTP 429 会停止本次兑换；稍后可重新开始。
- 官网返回的玩家信息与核对结果不一致时，停止当前账号并继续下一个账号。
- 超时或网络错误时，停止当前账号并继续下一个账号。
...
备份不包含公开码缓存或临时运行状态。导入时先校验并预览数量，然后按 `serverId + playerId` 合并账号、对兑换状态记录取并集；“成功”优先于“已使用”，不会删除本机已有历史。备份含有玩家 ID，请自行妥善保管，不要上传到公开 Issue。
