...
面板引用在结构化状态中保留原始种类。当前全员 ATK 与 DEF 均归一为 `1`：跨角色攻击比值按 `ATK_来源 / ATK_目标 = 1` 计入动作、角色、回合和全队 ATK 总倍率；防御转攻击在结算后从 `ATK×(DEF0_目标/ATK_目标)` 化简为 `%DEF`，并与 STR/MAG 一样按独立属性倍率汇总。
...
前端把 `sourceAttackOverTargetAttack` 显示为“系数 × ATK_来源 / ATK_目标”，标注数值来源，并额外显示“已按 ATK 比值1计入”。`targetBaseDefenseOverTargetAttack` 不显示长公式，而显示为结算后的“系数 % DEF”。不同 `sourceId` 或目标面板的项不能合并为同一条普通攻击加成；即使当前所有角色 ATK 归一化而数值恰好相同，仍须保留来源差异。穆瓦诺复制时将复制到的 `attackRate` 转为以被复制目标为 `sourceId` 的符号项；原施加者仍保留在复制状态的 Buff 来源字段，不能与数值来源混用。
