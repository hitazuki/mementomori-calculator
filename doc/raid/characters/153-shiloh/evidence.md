...
- `ActiveSkillMB` / `PassiveSkillMB`：按Lv240、EX3建模。“自动战斗・Boss战斗”不包含讨伐，因此讨伐第9回合前S1使用15%双Buff、被动153003使用10%攻击档；技能文本另列的“第9回合起”使两者恢复30%/50%高档。
...
| 被动153003 | roundStart，1/5/9回合 | 攻击+10%（第9回合起+50%） | topAttack×2 | 4次目标行动 |
| 被动153004 | actionStart，每4次自身行动 | 3个独立防御Buff | self + topAttackOther×2 | 4次目标行动 |
| S1 | beforeDamage | 暴击率/暴击伤害+15%（第9回合起+30%） | topAttack×2 | 4次目标行动；本次伤害前生效 |
...
- S1：确认第1/5回合使用15%两个低档EffectGroup，第9回合切为30%高档；暴击关闭时暴伤不生效但Buff仍存在。
