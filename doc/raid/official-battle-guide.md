...
| 类型 | 官方语义 | 当前拉表 |
| --- | --- | --- |
| 物理攻击 | 受物理防御和物理暴击伤害降低影响 | 使用 `phys`，结算Boss的防御与物防路径 |
| 魔法攻击 | 受魔法防御和魔法暴击伤害降低影响 | 使用 `mag`，结算Boss的防御与魔防路径 |
| 直接攻击 | 无视防御力、物理防御力和魔法防御力 | 使用 `direct`，防御倍率固定为1；A.A.日志确认魔力直接攻击可暴击并受目标承伤降低影响 |

不能仅因伤害基准为STR或其他能力值就自动判断为直接攻击；应以技能描述和子技能类型为准。

### 7.1 A.A.日志确认的直接攻击结算

`data/battle-logs/41-60-d.json` 中，A.A.为 `UnitId=48 / BattleCharacterGuid=41`，基础魔力为 `9,260,517`。以下记录均能按整数伤害精确闭合：

```text
强化普攻暴击：9,260,517 × 330% × (1.5 + 1.98 + 0.20) = 112,459,718
S2暴击：       9,260,517 × 980% × (1.5 + 1.98)        = 315,820,671
强化普攻非暴击：9,260,517 × 330% × (1 - 30%)          = 21,391,794
```

- 强化普攻与S2的暴击结果均为 `HitType=3`；帕拉底亚的暴击监听计数也随之增加，确认魔力直接攻击属于可暴击攻击。
- 第三式的目标持有 `550405800330101`“承伤降低30%”，且结果不含防御衰减，确认直接攻击无视双防但仍进入目标承伤增减管线。
- 该日志没有“同目标、同暴击状态、只切换攻击者输出增伤”的受控样本，因此只直接证明目标承伤降低；攻击者输出增伤与目标承伤增加沿用当前统一 `damageRate` 口径，二者同时存在时是否游戏原生加算仍待专门日志确认。
- 以上证据直接覆盖A.A.的 `%MAG` 直接攻击。光士的 `%STR` 同样声明为直接攻击，当前复用同一通用管线，但尚无光士自身的受控日志。
...
- アイリーン为 `UnitId=109 / BattleCharacterGuid=21`。`45-57-g` 的BattleType为Boss战，因此S1 `ActiveSkillId=109001` 在伤害前写入 `EffectGroupId=10900120101 / SkillCategory=5 / EffectTurn=4`，只能证明Boss战采用不可解除的防御力降低25%。MB描述没有把讨伐列入该条件；讨伐实现应采用默认的 `10900120103 / SkillCategory=3` 可解除25%档。
...
- 小白的S1与回合开始被动都写明高档在“自动战斗・Boss战，或第9回合起”生效。讨伐不属于前两种模式，因此第1～8回合使用15%暴击率/暴击伤害与10%攻击力，第9回合起才使用30%与50%。`y49-50` 的高档EffectGroup来自其BattleType对应模式，只用于确认目标、阶段和持续时间，不能据此把讨伐提前判为高档。
- ［仲夏日之梦］阿姆蕾特S1的防御降低默认是25%、2回合；描述只在“自动战斗・Boss战”延长至4回合，讨伐不命中该条件。无限之塔的50%是另一独立模式档。
- 战斗模式名称必须按MB描述逐项匹配；“Boss战”与讨伐是不同类型，不得因为目标同为单体Boss而合并分支。
