  ...
  <article class="card rating-card">
    ...
    <template v-else-if="rating">
      ...
      <div class="rating-content">
        ...
        <ol class="rating-reasons">
          <li v-for="(axis, index) in rating.axes" :key="axis.key">
            <button class="rating-axis" :class="{ selected: activeAxis === index }" :aria-pressed="activeAxis === index" @click="activeAxis = index; activeTag = null">
      ...
      <section class="rating-tags">
        <h3>{{ t('ratingTags') }}</h3>
        <p class="rating-caption">{{ t('ratingTagsHelp') }}</p>
        <div v-for="group in tagGroups" :key="group.key" class="rating-tag-group">
          <strong>{{ t('ratingGroup_' + group.key) }}</strong>
          <div><button v-for="tag in group.tags" :key="tag.key" class="rating-tag" :aria-pressed="activeTag === tag.key" @click="activeTag = activeTag === tag.key ? null : tag.key">
...
const freshness = ref('unknown'), activeAxis = ref(0), activeTag = ref(null)
const cache = new Map()
...
watch(() => props.character.id, () => { activeAxis.value = 0; activeTag.value = null })
...
const evidenceTitle = computed(() => activeTag.value ? RATING_TAGS[activeTag.value].label : t('ratingAxis_' + rating.value?.axes[activeAxis.value].key))
const evidence = computed(() => {
  const refs = activeTag.value ? rating.value?.tags.find(tag => tag.key === activeTag.value)?.evidence : rating.value?.axes[activeAxis.value].evidence
  return rating.value?.sources.filter(source => refs?.includes(source.key)) ?? []
})
...
.rating-tag { border:1px solid var(--border-subtle); background:transparent; color:var(--text-primary); border-radius:8px; padding:5px 8px; cursor:pointer; text-align:left; font:inherit; }
.rating-tag[aria-pressed="true"],.rating-tag:hover { border-color:var(--gold); background:var(--gold-dim); }
