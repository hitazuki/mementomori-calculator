    ...
    <template v-else>
      ...
      <template v-else>
        ...
        <div class="catalog-detail">
          ...
          <section class="catalog-skills"><article v-for="skill in selected.skills" :key="skill.id" class="card catalog-skill"><header><CharacterCatalogImage :path="`images/skills/${skill.id}.png`" :fallback="skill.slot"/><div><small>{{ skill.slot }}<template v-if="skill.cooldown != null"> · {{ t('raidCharacterCooldownValue', { n: skill.cooldown }) }}</template></small><h3>{{ skill.name }}</h3></div></header><p class="catalog-level-note">{{ t('catalogLevelNote') }}</p><div v-for="(level, index) in skill.levels" :key="level.level" class="catalog-level" :class="{ 'catalog-max-level': index === skill.levels.length - 1 }"><strong>{{ t('raidCharacterMbSkillLevel', { n: level.level }) }}<span v-if="index === skill.levels.length - 1"> · {{ t('catalogLatest') }}</span></strong><small> · {{ t('catalogUnlock', { n: level.unlockLevel }) }}</small><p class="catalog-skill-text">{{ level.text }}</p></div></article><article v-if="selected.exclusiveEffects?.length" class="card catalog-skill"><h3>{{ t('catalogExclusive') }}</h3><div v-for="effect in selected.exclusiveEffects" :key="effect.level" class="catalog-level"><strong>{{ t('raidCharacterMbExclusiveLevel', { n: effect.level }) }}</strong><p class="catalog-skill-text">{{ effect.text }}</p></div></article></section>
        </div>
      </template>
