    ...
    <template v-else>
      ...
      <template v-else>
        ...
        <div class="catalog-detail">
          ...
          <section class="catalog-skills"><h2>{{ t('catalogSkills') }}</h2><article v-for="skill in selected.skills" :key="skill.id" class="card catalog-skill"><header><CharacterCatalogImage :path="`images/skills/${skill.id}.png`" :fallback="skill.slot"/><div><small>{{ skill.slot }}<template v-if="skill.cooldown != null"> · {{ t('raidCharacterCooldownValue', { n: skill.cooldown }) }}</template></small><h3>{{ skill.name }}</h3></div></header><p class="catalog-skill-text">{{ latest(skill)?.text }}</p><details v-if="skill.levels.length > 1"><summary>{{ t('catalogLevels') }}</summary><div v-for="(level, index) in skill.levels" :key="index" class="catalog-level"><strong>{{ t(level.type === 'exclusive' ? 'raidCharacterMbExclusiveLevel' : 'raidCharacterMbSkillLevel', { n: level.level }) }}</strong><small v-if="level.type !== 'exclusive'"> · {{ t('catalogUnlock', { n: level.unlockLevel }) }}</small><p class="catalog-skill-text">{{ level.text }}</p></div></details></article></section>
        </div>
      </template>
...
const latest = skill => skill.levels.find(level => level.type === 'level') ?? skill.levels[0]
