...
import { DEF_BENCHMARKS, LEVEL_PRESETS } from '../constants/levelTable.js'
import { COMPARE_BUILDS_DEFAULT } from '../constants/presets.js'
...
let cs = {
  builds: JSON.parse(JSON.stringify(COMPARE_BUILDS_DEFAULT)),
  benchmarks: JSON.parse(JSON.stringify(DEF_BENCHMARKS.slice(0,4))),
...
const METRICS = {
  dmgRatePct: { label:'综合穿透率%', fmt: v=>`${v.toFixed(1)}%`,    unit:'%' },
  finalDmg:   { label:'最终伤害',    fmt: v=>fmt(v),                 unit:''  },
  defMitPct:  { label:'防御路减伤%', fmt: v=>`${v.toFixed(1)}%`,    unit:'%' },
  pmMitPct:   { label:'物魔路减伤%', fmt: v=>`${v.toFixed(1)}%`,    unit:'%' },
}

export function renderCompare(container) {
  container.innerHTML = `
  <div class="view-header animate-fadeup">
    <h1 class="view-title">⚖ 多方案配装对比</h1>
    <p class="view-desc">在不同防御档位下，同时对比最多6套穿透配装的伤害输出差异</p>
  </div>
  <div class="grid-sidebar animate-fadeup" style="align-items:start;gap:16px">
    <div class="flex-col gap-12">
      <div class="card">
        <div class="card-title">⚙ 公共基础参数</div>
        <div class="form-group">
          <label class="form-label">面板攻击力 <span class="value-display" id="cv-atk">${fmt(cs.baseAtk)}</span></label>
          <input class="form-input" type="number" id="ci-baseAtk" value="${cs.baseAtk}" min="0">
        </div>
        <div class="form-group">
          <label class="form-label">技能倍率 <span class="value-display" id="cv-coeff">${(cs.skillCoeff*100).toFixed(0)}%</span></label>
        ...
        <div class="form-group">
          ...
          <div style="display:flex;gap:8px">
            <button class="btn ${cs.damageType==='phys'?'btn-primary':'btn-ghost'} btn-sm" style="flex:1" data-type="phys">物理 (P.DEF)</button>
            <button class="btn ${cs.damageType==='mag' ?'btn-primary':'btn-ghost'} btn-sm" style="flex:1" data-type="mag">魔法 (M.DEF)</button>
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">攻击方等级 (影响贯通定数)</label>
          <select class="form-select" id="ci-atkLevelPreset">
            <option value="-1" ${cs.atkLevelPresetIdx===-1?'selected':''}>当前自定义系数</option>
        ...
        <div class="form-group">
          ...
          <select class="form-select" id="ci-defLevelPreset">
            <option value="-1" ${cs.defLevelPresetIdx===-1?'selected':''}>当前自定义系数</option>
      ...
      <div class="card">
        <div class="card-title">🔧 配装方案 <span class="text-xs text-muted">（最多6套）</span></div>
        <div id="cmp-builds" class="flex-col gap-8"></div>
        <button class="btn btn-secondary btn-sm mt-8 w-full" id="cmp-addBuild" ${cs.builds.length>=6?'disabled':''}>+ 添加方案</button>
      </div>

      <div class="card">
        <div class="card-title">🏹 对标防御档位</div>
        <div id="cmp-benchmarks" class="flex-col gap-8"></div>
        <button class="btn btn-ghost btn-sm mt-8 w-full" id="cmp-addBench">+ 添加档位</button>
      </div>
    </div>
    ...
    <div class="flex-col gap-12">
      <div class="card">
        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px">
          <div style="display:flex;gap:6px">
            ${['bar','radar','table'].map(m=>`<button class="btn ${cs.chartMode===m?'btn-primary':'btn-ghost'} btn-sm" data-mode="${m}">${{bar:'柱状图',radar:'雷达图',table:'数据表'}[m]}</button>`).join('')}
          </div>
      ...
      <div class="card">
        <div class="card-title">📊 vs 首方案增减值</div>
...
function renderBenchRows(container) {
  ...
  el.innerHTML = cs.benchmarks.map((b,i) => `
    <div class="flex items-center gap-8">
      ...
      <input class="form-input" type="number" data-bench="${i}" data-field="def" value="${b.def}" style="flex:1" placeholder="防御力(DEF)">
      <input class="form-input" type="number" data-bench="${i}" data-field="pmDef" value="${b.pmDef}" style="flex:1" placeholder="物理/魔法防御">
...
function attachCompareListeners(container) {
  ...
  container.querySelector('#cmp-addBuild')?.addEventListener('click', () => {
    ...
    cs.builds.push({ id:Date.now(), name:`方案${cs.builds.length+1}`, pen:18950, pmPen:31200, dmgBonus:0.3, defDebuff:0, factionBonus:1.0 })
  ...
  container.querySelector('#cmp-addBench')?.addEventListener('click', () => {
    cs.benchmarks.push({ label:`档位${cs.benchmarks.length+1}`, def:5_000_000, pmDef:5_000_000 })
...
function refreshCompare() {
  ...
  if (cs.chartMode === 'table') {
    ...
    tableDiv.innerHTML = `<table class="data-table"><thead><tr><th>方案</th>${hdrs}</tr></thead><tbody>${
  ...
  if (cs.chartMode === 'bar') {
    compareChart.setOption({
      ...baseChartOption('配装对比 · ' + metric.label),
    ...
    compareChart.setOption({
      ...baseChartOption('雷达对比 · ' + metric.label),
      radar: {
...
function renderDelta(results, metric) {
  ...
  const hdrs = cs.benchmarks.map(b=>`<th>${b.label}</th>`).join('')
  el.innerHTML = `<table class="data-table"><thead><tr><th>对比</th>${hdrs}</tr></thead><tbody>${
