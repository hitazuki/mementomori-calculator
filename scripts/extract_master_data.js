/** ½Å±¾: extract_master_data.js
 * ÓÃÍ¾: ´ÓÓÎÏ·Ö÷Êý¾Ý(Master Data)ÖÐÌáÈ¡²¢´¦ÀíËùÐèµÄ×Öµä¡¢±¾µØ»¯ÎÄ±¾µÈ£¬Éú³ÉÇáÁ¿¼¶µÄ JSON Êý¾Ý¹©ÏîÄ¿Ê¹ÓÃ¡£
 */
import fs from 'fs';
...
for (const cl of colLevelsMB) {
  if (cl.CharacterRarityFlags === 512) {
    ...
    for (const p of baseInfos) {
      ...
      let key = '';
      if (ptype === 1) key = 'appBasicStr'; // è…•åŠ›
      else if (ptype === 2) key = 'appBasicDex'; // æŠ€åŠ?      else if (ptype === 3) key = 'appBasicMag'; // é­”åŠ›
      else if (ptype === 4) key = 'appBasicStm'; // è€ä¹…åŠ?      else key = `[UnknownBaseParam_${ptype}]`;
