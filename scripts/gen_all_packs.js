...
for (const [key, packs] of Object.entries(ultraSaleRawDict)) {
  ...
  const price = parseInt(priceStr.replace('楼', ''))
...
const mdLines = [
  '<!-- 鑷姩鐢熸垚锛岃鍕挎墜鍔ㄤ慨鏀规鏂囦欢 -->',
  '<!-- 绀煎寘鏁版嵁婧愬湪 src/constants/witchGiftPacks.json 涓淮鎶?-->',
  '',
  '# 榄斿コ鐨勮禒绀?(Witch\'s Gift) 绀煎寘璁板綍',
  '',
  '鏈枃妗ｇ敤浜庤褰曗�滈瓟濂崇殑璧犵ぜ鈥濆悇涓樁娈佃Е鍙戠殑绀煎寘鍐呭涓庝环鏍笺�?,
  ''
]

function formatItem(item) {
  let name = idToNameMap.get(`${item.ItemType}_${item.ItemId}`) || `鏈煡鐗╁搧[${item.ItemType},${item.ItemId}]`
  let count = item.ItemCount
  if (count >= 100000 && count % 10000 === 0) count = (count / 10000) + '涓?
  return `${name} x${count}`
}

function generateTable(packs, title) {
  mdLines.push(`## ${title}`)
  mdLines.push('| 搴忓彿 | 瑙﹀彂鏉′欢 / 闃舵 | 浠锋牸 | 鍐呭鐗?1 | 鍐呭鐗?2 | 鍐呭鐗?3 | 澶囨敞 |')
  mdLines.push('| :--- | :--- | :--- | :--- | :--- | :--- | :--- |')
...
generateTable(firstFour, '棣栨 - 鍥涘睘锛堣摑/绾?缈?榛勶級')
generateTable(firstLightDark, '棣栨 - 鍏夋殫')
generateTable(rerunFour, '澶嶅埢 - 鍥涘睘锛堣摑/绾?缈?榛勶級')
generateTable(rerunLightDark, '澶嶅埢 - 鍏夋殫')
