...
**Status-class rule:** `removableBuff` / `unremovableState` is a combat behavior classification, not a judgment about whether the effect changes the multiplier. Do not infer it from a shield, defense, damage-reduction, `EffectTurn`, `IsHide`, or `RemoveEffectType`. A positive EffectGroup is a `removableBuff` unless its game text explicitly says it cannot be removed; only that explicit wording permits `unremovableState`. Confirm and record `SkillCategory` from a combat log whenever one is available, including caster, target, granter, and phase.
